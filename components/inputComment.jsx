...
const InputComment = ({user,idPost,post}) => {
    ...
    const submitComment= async (e) => {
        ...
        setIsLoading(true)
          const data = {
            ...
            author: user._id,
            post:idPost,
            creator_of_post: post.author._id,
            date: new Date
          }
          try {
    ...
    return (
        <>
          {user !== ''?(
              <form onSubmit={submitComment} action="" method="post" className='flex flex-col justify-between h-44  shadow-md bg-white'>
          ...
          <div className='flex  items-center px-4 w-full'>
              <input  type="text" id='post-input-home' minLength="5" required className='w-full py-2 ' value={inputValue} onChange={handleInput} placeholder={user !== ""?`Leave a comment ${user.first_name}!`:"What's on your mind..."} />
          </div>
          </div>
